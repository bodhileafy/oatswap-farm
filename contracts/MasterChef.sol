...
interface IMigratorChef {
    // Perform LP token migration from legacy OatSwap to OatSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to OatSwap LP tokens.
    // OatSwap must mint EXACTLY the same amount of OatSwap LP tokens or
    // else something bad will happen. Traditional OatSwap does not
